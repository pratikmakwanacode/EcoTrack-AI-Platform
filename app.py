...
# ──────────────────────────────────────────────────────────────────────────
   ...
   st.markdown(f"""<div style="{highlight_border} border-radius: 8px; padding: 10px 15px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;"><div style="display: flex; align-items: center; gap: 10px;"><span style="font-weight: 800; font-size: 1rem; color: {'#10b981' if is_current else '#94a3b8'};">#{row['rank']}</span><span style="font-weight: bold; color: #ffffff;">{row['username']}</span>{"<span style='background: #065f46; color: #34d399; font-size: 0.6rem; padding: 2px 6px; border-radius: 9999px; font-weight: bold;'>YOU</span>" if is_current else ""}</div><div style="text-align: right;"><div style="font-weight: 800; color: #10b981; font-size: 0.95rem;">{row['points']} pts</div><div style="font-size: 0.7rem; color: #64748b;">Avg CO₂: {row['avg_co2']:,} kg</div></div></div>""", unsafe_allow_html=True)
